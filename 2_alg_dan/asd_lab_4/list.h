...
public:
    ...
    void erase (const iterator<V> &i);                    // удалить указанный (в позиции)
...
list<V>::list(const list<V>& other)
{
    ...
    for (iterator<V> i=iterator<V>(other.head); !i.isnull(); ++i) { push_back(*i); }
}
template <typename V>
...
template<class V>
void list<V>::erase(const iterator<V> &i)
{
    if (size > 0)
    {
        node *it = i.nit;
