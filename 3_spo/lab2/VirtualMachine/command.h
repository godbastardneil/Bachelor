...
{
public:
        virtual void operator() (CPU &processor) = 0;
        virtual ~Command() = 0;
};
...
public:
    virtual void operator()(CPU &processor) noexcept override final;
        ...
        class ArithmInt : public Command
        {
            ...
            virtual int32_t calculation(int parametr1, int parametr2) = 0;

        public:
            virtual void operator()(CPU &processor) override;
        };

class AddI : public ArithmInt //8
{
    virtual int calculation(int parametr1, int parametr2) override final
    {
        int64_t res = int64_t(parametr1) + int64_t(parametr2);
        if ((res > std::numeric_limits<int32_t>::max()) || (res < std::numeric_limits<int32_t>::min())) { throw std::overflow_error("AddI overflow"); }
...
class SubI: public ArithmInt //9
{
    ...
    {
        int64_t res = int64_t(parametr1) - int64_t(parametr2);
        if ((res > std::numeric_limits<int32_t>::max()) || (res < std::numeric_limits<int32_t>::min())) { throw std::overflow_error("SubI overflow"); }
        return static_cast<int32_t>(res);
    }
};
class MultI : public ArithmInt //10
{
    ...
    {
        int64_t res = int64_t(parametr1) * int64_t(parametr2);
        if ((res > std::numeric_limits<int32_t>::max()) || (res < std::numeric_limits<int32_t>::min())) { throw std::overflow_error("MultI overflow"); }
        return static_cast<int32_t>(res);
    }
};
class DivI : public ArithmInt //11
{
    virtual int calculation(int parametr1, int parametr2) override final
    {
        if ((parametr1 == std::numeric_limits<int32_t>::min()) && (parametr2 == -1)) { throw std::overflow_error("DivI overflow"); }
        if (parametr2 == 0) { throw std::invalid_argument("zero in the denominator"); }
        ...
        class ArithmFLong : public Command
        {
            ...
            virtual float calculation(float parametr1, float parametr2) = 0;

        public:
            virtual void operator()(CPU &processor) override final;
        };
...
{
    virtual float calculation(float parametr1, float parametr2) override final
    {
        if (fabsf(parametr2) <= std::numeric_limits<float>::epsilon()) { throw std::invalid_argument("zero in the denominator"); }
        return parametr1 / parametr2;
    }
};
