...
{
    // -- проверки -- //
    void CheckForBinary() const;    // -- цифры должны быть четверичными -- //
...
public:
    ...
    bit() = default;
    explicit bit(const size_type& n, value_type t = 0) : Array(n,t) { CheckForBinary(); }
    bit(const std::initializer_list<value_type> &t) : Array(t) { CheckForBinary(); }
    bit(const std::string &str) : Array(str) { CheckForBinary(); }
